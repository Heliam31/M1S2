...
class Noeud:
    ...
    def lookup (self,ip,port, id):
        ...
        if (self.is_child(id)):
            ...
            json_send(self.TableVois[(self.key+1)% 65536][0],self.TableVois[(self.key+1)% 65536][1],sendaj)
            self.nb_query += 1
    ...
    def join(self, ipS, portS, ip, port):
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as serversocket:
            ...
            if (json_data['request'] == 'OK'):
                ...
                self.key = json_data['id']
                self.TableVois[(self.key+1)% 65536] = [0,0,0]
                ...
                print("[JOIN] : mon ipSuiv ", self.TableVois[(self.key+1)% 65536][0])
                print("[JOIN] : mon PortSuiv ", self.TableVois[(self.key+1)% 65536][1])
                print("[JOIN] : mon idSuiv ", self.TableVois[(self.key+1)% 65536][2])
                print("[JOIN] : mon id: ", self.key)
                print("[JOIN] : mes données: ", self.data)
        self.create()
        print("ma table de voisinage : ", self.TableVois)
        self.nb_management += 1
    ...
    def update(self, serversocket, ip_new, port_new, id_new):
        ...
        j = self.TableVois[(self.key+(65536/2))% 65536][2]
        if ((self.key < id_new and j<id_new and self.key < j  ) or (self.key > id_new and j>id_new and self.key < j  ) or (self.key > id_new and j<id_new and self.key > j  )):
            print("[UPDATE] : update des tables terminées")
        else:
            i=1
            cpt=1
            while i <= (65536/2)% 65536 :
                
                self.whois(self.ip, self.port, ((self.key + i)% 65536))
                sortie = 0
                serversocket.listen(5)
                print('listening on port:', serversocket.getsockname()[1])
                while sortie == 0:
                    (clientsocket, address) = serversocket.accept()
                    print("[UPDATE/WHOIS] : nouvelle connexion")
                    json_data = json_recv(clientsocket)
                    print(json_data['request'])
                    if (json_data['request'] == 'IAM'):
                        print("[UPDATE/WHOIS] : Réponse à notre demande")
                        self.TableVois[json_data['id_wanted']] = [json_data['ip'],json_data['port'],json_data['id']]
                        sortie = 1
                i=2**cpt
                cpt+=1
